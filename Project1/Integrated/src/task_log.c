...
void *task_log(void *param)
{
  ...
  /* Process the log queue */
	while (status == SUCCESS)
	{
      ...
      /* Request for the msg from queue */
        ...
        log_msg = info.data;
          ...
          /* Log the data received */
				  case LOG_MSG:
					  ...
					  // DEBUG ("[DEBUG] Timestamp: %s", ctime(&log_msg.timeStamp));
...
/* Function to log data in the file */
Status_t log_data(FILE **pfile, Message_t *message, const char *fileName)
{
  /* Error Handling */
  ...
  /* Write the log string in the file */
